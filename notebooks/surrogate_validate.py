...
def validate_all_ones(surrogate, loss_fn, val_data, num_classes, validation_batch_size=None):
    '''
    Calculate mean validation loss when S is all ones: no masking.
    Useful for comparing to original model
...
# ).softmax(dim=1).cpu().data.numpy()
            ...
            ##TODO: this should be an array of matrices filled with all ones. Size of matrix copy from validate method in FastShap training
         # num_classes = targets.max() + 1
        
        # Setup for training.
    #surrogate = self.surrogate
    device = next(surrogate.surrogate.parameters()).device    
    S_val = torch.ones(len(val_data), surrogate.num_players, device=device)
    print(S_val.shape)
